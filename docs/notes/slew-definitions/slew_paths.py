...
results. This is a script for the report, not library code.
"""
...
from pulseq_reports.seq_utils import gradient_points, iter_blocks
...
def limit_check_slew(seq: pp.Sequence) -> dict:
    ...
    - "segments": rows (t0, t1, slope) for each segment of each gradient event, the
      polyline of the event as the event stores it (pulseq_reports.seq_utils).
    ...
    for _, start, duration, block in iter_blocks(seq):
        for ax in AXES:
            g = getattr(block, f"g{ax}", None)
            first = last = 0.0
            if g is not None:
                t, amp = gradient_points(g, start)
                out[ax]["events"].append((t, amp / GAMMA))
                dt = np.diff(t)
                keep = dt > 1e-12
                slope = np.diff(amp)[keep] / dt[keep] / GAMMA
                out[ax]["segments"] += list(zip(t[:-1][keep], t[1:][keep], slope, strict=True))
            ...
            prev_last[ax] = last
    for d in out.values():
        d["segments"] = np.array(d["segments"]).reshape(-1, 3)
...
def octave_command() -> list:
    """octave-cli from PATH, or from the project's locked nixpkgs."""
...
def compute(
    ...
    False and out_dir already has their output), and compute the pypulseq paths.
    ...
    for name, build in EXAMPLES.items():
        build().write(str(out_dir / f"{name}.seq"))
